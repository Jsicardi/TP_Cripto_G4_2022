...
bool invert_lsbi_message_bits(BmpBody body,BinaryMessage *msg, FILE * origin_fd,uint8_t (*pattern_functions[]) (uint8_t bit)){
    ...
    for(uint32_t i = 0; i < body.pixel_count * 3; i++){
        ...
        if(bit_position == BITS_IN_BYTE-1){
            set_bit_at(aux_msg.curr_byte_ptr - 1,bit_position,pattern_functions[pattern](message_bit));
        }
        else{
            set_bit_at(aux_msg.curr_byte_ptr,bit_position,pattern_functions[pattern](message_bit));
        }
        bit_position++;
